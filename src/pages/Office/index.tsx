...
import {
  createDefaultLayout,
  migrateLayoutColors,
} from '@/lib/pixel-office/layout/layoutSerializer';
import {
  playDoneSound,
  playBackgroundMusic,
  stopBackgroundMusic,
...
// Removed unused formatMs and MiniSparkline utils
...
function getGhostBorderDirection(
  col: number,
  row: number,
  cols: number,
  rows: number
...
export function Office() {
  ...
  > | null>(null);
  const [cachedPlatformTestResults, setCachedPlatformTestResults] = useState<Record<
    string,
    PlatformTestResult | null
  > | null>(null);
  const [cachedSessionTestResults, setCachedSessionTestResults] = useState<Record<
    string,
    AgentSessionTestResult | null
  > | null>(null);
  const [cachedDmSessionResults, setCachedDmSessionResults] = useState<Record<
    string,
    PlatformTestResult | null
  > | null>(null);
  ...
  const [subagentCreatorInfo, setSubagentCreatorInfo] = useState<{
    parentAgentId: string;
    x: number;
    y: number;
  } | null>(null);
  const [serverTooltip, setServerTooltip] = useState<{ open: boolean; x: number; y: number }>({
    open: false,
    x: 0,
    y: 0,
  });
  const idleRankRef = useRef<Array<{
    agentId: string;
  ...
  const floatingCommentsRef = useRef<
    Array<{ key: string; text: string; x: number; y: number; opacity: number }>
  >([]);
  ...
  const fetchVersionInfo = useCallback(async (forceLatest = false) => {
    ...
    try {
      ...
      const data = await hostApiFetch<ReleaseInfo>(url);
    ...
    const loadLayout = async () => {
      if (cachedOfficeState) {
        ...
        if (!spriteAssetsPromise) {
          spriteAssetsPromise = Promise.all([loadCharacterPNGs(), loadWallPNG()]).then(
            () => undefined
          );
        }
      ...
      try {
        ...
        const data = await invokeIpc<any>('office:get-layout');
        if (data.layout) {
      ...
      if (ctx) {
        ...
        for (const ch of office.getCharacters()) {
          ...
          for (let i = 0; i < ch.codeSnippets.length; i++) {
            ...
            const alpha =
              progress < 0.15 ? progress / 0.15 : progress > 0.88 ? (1 - progress) / 0.12 : 1;
            codeItems.push({
        ...
        const tickInterval = hasFloating
          ? isMobileViewport
            ? FLOATING_TICK_INTERVAL_MOBILE_MS
            : FLOATING_TICK_INTERVAL_DESKTOP_MS
          : 180;
  ...
  useEffect(() => {
    ...
    const mockWeeks = Array.from({ length: 52 }, () => ({
      days: Array.from({ length: 7 }, () => ({
        count: Math.random() < 0.8 ? 0 : Math.floor(Math.random() * 3),
        date: '',
      })),
    }));
    ...
    // 异步拉取真实数据
    import('@/lib/api-client').then(({ invokeIpc }) => {
      invokeIpc<any>('office:get-github-contributions')
        .then((data) => {
          if (data && data.weeks && !data.error) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { hostApiFetch } = await import('@/lib/host-api');
        const data = await hostApiFetch<{ agents: Array<{ agentId: string; grid: number[][] }> }>(
          '/api/activity-heatmap'
        );
        if (data.agents) activityHeatmapRef.current = data.agents;
      } catch {}
    })();
  }, []);
  ...
  useEffect(() => {
    const preloadIdleRank = async () => {
      try {
        ...
        if (data && data.agents) idleRankRef.current = data.agents;
      } catch {
        // ignore
      }
    };
    void preloadIdleRank();
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchAgents = async () => {
      try {
        const { hostApiFetch } = await import('@/lib/host-api');
        const data = await hostApiFetch<any>('/api/agent-activity');
        const newAgents: AgentActivity[] = (data.agents || []).map((agent: any) => ({
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        const { hostApiFetch } = await import('@/lib/host-api');
        const { buildProviderListItems } = await import('@/lib/provider-accounts');

        const [accounts, vendors] = await Promise.all([
          hostApiFetch<any[]>('/api/provider-accounts'),
          hostApiFetch<any[]>('/api/provider-vendors'),
        ]);

        console.log('Fetched accounts:', accounts);
        console.log('Fetched vendors:', vendors);

        const items = buildProviderListItems(accounts, [], vendors, null);
        console.log('Built provider items:', items);
        console.log('First item account:', items[0]?.account);
        console.log('First item vendor:', items[0]?.vendor);

        const result = items.map((item) => ({
          id: item.account.id,
          api: item.vendor?.apiKeyUrl || '',
          models: item.account.model
            ? [
                {
                  id: item.account.model,
                  name: item.vendor?.model || item.account.model,
        ...
        console.log('Final providers result:', result);
        console.log('Providers length:', result.length);
        console.log('First provider:', result[0]);
        providersRef.current = result;
      } catch (error) {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const data = await hostApiFetch<any>('/api/config');
        console.log('Received /api/config data:', data);
        const map = new Map<
          string,
          {
            sessionCount: number;
        ...
        if (data.providers) {
          console.log('Config providers:', data.providers);
          providersRef.current = data.providers;
          const accessModeMap: Record<string, 'auth' | 'api_key'> = {};
          for (const provider of data.providers) {
            if (
              provider?.id &&
              (provider.accessMode === 'auth' || provider.accessMode === 'api_key')
            ) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const { invokeIpc } = await import('@/lib/api-client');
      await invokeIpc('office:save-layout', office.layout);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const { col, row, worldX, worldY } = mouseToTile(
      e.clientX,
      e.clientY,
      canvasRef.current,
      office,
    ...
    if (editor.isEditMode) {
      ...
      if (editor.activeTool === EditTool.FURNITURE_PLACE) {
        ...
        if (entry) {
          ...
          editor.ghostValid = canPlaceFurniture(
            office.layout,
            editor.selectedFurnitureType,
            col,
            placementRow
          );
        }
        ...
        if (
          editor.isDragging &&
          col >= 0 &&
          col < office.layout.cols &&
          row >= 0 &&
          row < office.layout.rows
        ) {
          if (editor.activeTool === EditTool.TILE_PAINT) {
            applyEdit(
              paintTile(office.layout, col, row, editor.selectedTileType, editor.floorColor)
            );
      ...
      const onCamera = office.layout.furniture.some((f) => {
        ...
        return (
          tileX >= f.col &&
          tileX < f.col + entry.footprintW &&
          tileY >= f.row &&
          tileY < f.row + entry.footprintH
        );
      });
      ...
      const onPhoto =
        photographRef.current && tileX >= 10 && tileX < 17 && tileY >= -0.5 && tileY < 1;
      const onHeatmap =
        contributionsRef.current &&
        contributionsRef.current.username !== 'mock' &&
        tileX >= 1 &&
        tileX < 10 &&
        tileY >= -0.5 &&
        tileY < 1;
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (!editor.isEditMode) {
      ...
      if (e.button === 0) {
        ...
        const { worldX, worldY } = mouseToTile(
          e.clientX,
          e.clientY,
          canvasRef.current,
          office,
          ...
          if (!activityHeatmapRef.current) {
            (async () => {
              try {
                const { hostApiFetch } = await import('@/lib/host-api');
                const data = await hostApiFetch<{
                  agents: Array<{ agentId: string; grid: number[][] }>;
                }>('/api/activity-heatmap');
                if (data.agents) activityHeatmapRef.current = data.agents;
              } catch {}
            })();
    ...
    if (e.button === 0) {
      ...
      if (tool === EditTool.TILE_PAINT || tool === EditTool.WALL_PAINT || tool === EditTool.ERASE) {
        ...
        if (col >= 0 && col < office.layout.cols && row >= 0 && row < office.layout.rows) {
          ...
          editor.activeTool =
            editor.activeTool === EditTool.EYEDROPPER ? EditTool.TILE_PAINT : editor.activeTool;
          forceEditorUpdate();
        }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'r' || e.key === 'R') {
        if (editor.selectedFurnitureUid) {
          applyEdit(
            rotateFurniture(office.layout, editor.selectedFurnitureUid, e.shiftKey ? 'ccw' : 'cw')
          );
        }
  ...
  const getHoveredAgentInfo = useCallback(() => {
    ...
    if (hoveredCharacter?.isSubagent && hoveredCharacter.parentAgentId != null) {
      let parentAgentId: string | null = null;
      for (const [aid, cid] of map.entries()) {
        if (cid === hoveredCharacter.parentAgentId) {
          parentAgentId = aid;
          break;
        }
      }
  ...
  const renderAgentChip = (agent: AgentActivity, mobileGrid = false) => {
    ...
    const tempWorkerOwner = isTempWorker
      ? agent.name.replace(new RegExp(`^${t('pixelOffice.tempWorker')}\\s*`), '') ||
        parentAgentIdFromKey
      : '';
    ...
    return (
      <div key={agent.agentId} className="group relative overflow-visible">
        <div
          className={`pixel-agent-chip inline-flex h-8 items-center overflow-hidden rounded-lg border transition-colors ${
        ...
        >
          ...
          {isTempWorker ? (
            ...
            >
              <span className={`${mobileGrid ? 'text-[10px]' : 'text-[12px]'} truncate`}>
                {t('pixelOffice.tempWorker')}
              </span>
              <span className={`${mobileGrid ? 'text-[10px]' : 'text-[12px]'} truncate`}>
                {tempWorkerOwner}
              </span>
            ...
            <span className={mobileGrid ? 'min-w-0 text-xs truncate' : 'text-sm'}>
              {agent.name}
            </span>
          )}
          {agent.state === 'working' && (
          ...
          {agent.state === 'idle' && (
            <span
              className={`pixel-agent-chip-state uppercase tracking-wider ${mobileGrid ? 'text-[9px] truncate' : 'text-[10px]'}`}
            >
              {t('pixelOffice.state.idle')}
            </span>
          )}
          {agent.state === 'offline' && (
            <span
  ...
  return (
    ...
    >
      ...
      {floatingCodeRef.current.map((fc) => (
        <div
          key={fc.key}
          className="absolute pointer-events-none z-40 whitespace-nowrap"
          style={{
            left: 0,
            top: 0,
            opacity: fc.opacity,
        ...
        >
          <span
            ...
            style={{
              ...
              border:
                fc.kind === 'sre' ? '1px solid rgba(127,29,29,0.85)' : '1px solid transparent',
            }}
          >
            {fc.text}
          </span>
          ...
          {displayAgents.length === 0 ? (
            ...
            <div className="flex gap-2 min-w-full snap-x snap-mandatory">
              {mobileAgentPages.map((page, pageIndex) => (
                <div
                  key={`mobile-agent-page-${pageIndex}`}
                  className="grid grid-cols-3 grid-rows-3 gap-2 min-w-full h-[8.4rem] shrink-0 snap-start"
                >
                  {page.map((agent) => renderAgentChip(agent, true))}
                  {page.length < 9 &&
                    Array.from({ length: 9 - page.length }).map((_, i) => (
                      <div
                        key={`mobile-agent-page-${pageIndex}-placeholder-${i}`}
      ...
      >
            ...
            {hoveredInfo.kind === 'gatewaySre' ? (
                ...
                <div className="space-y-0.5 text-[var(--text-muted)]">
                  <div className="flex justify-between gap-4">
                    ...
                    <span className="text-[var(--text)]">
                      {t(`pixelOffice.gatewaySre.status.${hoveredInfo.gatewaySre.status}`)}
                    </span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span>{t('pixelOffice.gatewaySre.responseMs')}</span>
                    <span className="text-[var(--text)]">
                      {hoveredInfo.gatewaySre.responseMs != null
                        ? `${hoveredInfo.gatewaySre.responseMs}ms`
                        : '--'}
                    </span>
                  </div>
              ...
              <>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="font-semibold text-[var(--text)]">
                    {hoveredInfo.isSubagent ? t('pixelOffice.tempWorker') : hoveredInfo.agent?.name}
                  </span>
                </div>
                {hoveredInfo.isSubagent ? (
                  ...
                  <div className="space-y-0.5 text-[var(--text-muted)]">
                    <div className="flex justify-between gap-4">
                      ...
                      <span className="text-[var(--text)]">
                        {hoveredInfo.stats?.sessionCount ?? '--'}
                      </span>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>{t('agent.messageCount')}</span>
                      <span className="text-[var(--text)]">
                        {hoveredInfo.stats?.messageCount ?? '--'}
                      </span>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>{t('agent.tokenUsage')}</span>
                      <span className="text-[var(--text)]">
                        {hoveredInfo.stats ? formatTokens(hoveredInfo.stats.totalTokens) : '--'}
                      </span>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>{t('agent.todayAvgResponse')}</span>
                      <span className="text-[var(--text)]">
                        {hoveredInfo.stats?.todayAvgResponseMs
                          ? `${(hoveredInfo.stats.todayAvgResponseMs / 1000).toFixed(1)}s`
                          : '--'}
                      </span>
                    </div>
                  </div>
        ...
        {serverTooltip.open &&
          ...
          (() => {
            ...
            const tooltipLeft = Math.max(
              ...
              Math.min(
                serverTooltip.x + 12,
                (containerRef.current?.clientWidth || 300) - (isMobileViewport ? 220 : 200)
              )
            );
              ...
              >
                ...
                <div className="space-y-0.5 text-[var(--text-muted)]">
                  ...
                  <div className="flex justify-between gap-4">
                    <span>{t('pixelOffice.gatewaySre.responseMs')}</span>
                    <span className="text-[var(--text)]">
                      {snapshot.responseMs != null ? `${snapshot.responseMs}ms` : '--'}
                    </span>
                  </div>
                  {snapshot.error && <div className="text-red-300">{snapshot.error}</div>}
                </div>
              </div>
            );
          })()}
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (
                    isMobileViewport &&
                    performance.now() - selectedAgentOpenedAtRef.current < 280
                  )
                    return;
                  setSelectedAgentId(null);
                }}
              >
                <div
                  className={modalPanelClass('w-[24rem]', 'max-h-[78%]')}
                  onClick={(e) => e.stopPropagation()}
                >
        ...
        {subagentCreatorInfo &&
          ...
          (() => {
            const tooltipLeft = Math.max(
              8,
              Math.min(subagentCreatorInfo.x + 12, (containerRef.current?.clientWidth || 300) - 220)
            );
        ...
        {showModelPanel && !isEditMode && (
          <div
            className={modalOverlayClass}
            onClick={() => {
              if (isMobileViewport && performance.now() - modelPanelOpenedAtRef.current < 280)
                return;
              setShowModelPanel(false);
            }}
          >
        ...
        {showTokenRank &&
          ...
          (() => {
            const ranked = agents
              .map((a) => ({
                ...a,
                tokens: agentStatsRef.current.get(a.agentId)?.totalTokens || 0,
              }))
            ...
            const handleCloseTokenRankOverlay = () => {
              ...
              if (isMobileViewport && performance.now() - tokenRankOpenedAtRef.current < 280)
                return;
              setShowTokenRank(false);
            };
            return (
              <div className={modalOverlayClass} onClick={handleCloseTokenRankOverlay}>
                <div
                  className={modalPanelClass('w-80', 'max-h-[78%]')}
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-semibold text-[var(--text)]">
          ...
          (() => {
            ...
            return (
              <div className={modalOverlayClass} onClick={() => setShowActivityHeatmap(false)}>
                <div
                  className={modalPanelClass('w-fit max-w-[94vw]', 'max-h-[85%]')}
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-semibold text-[var(--text)]">
                              ...
                              <svg width={svgW} height={svgH} className="block min-w-max">
                                ...
                                {dayLabels.map((label, d) => (
                                  <text
                                    key={d}
                                    x={leftPad - 4}
                                    y={topPad + d * (cellSize + gap) + cellSize / 2 + 3}
                                    textAnchor="end"
                                    fontSize={9}
                                    fill="var(--text-muted)"
                                  >
                                ...
                                {grid.map((row, d) =>
                                  row.map((v, h) => {
                                    const level =
                                      v === 0 ? 0 : Math.min(4, Math.ceil((v / maxVal) * 4));
                                    return (
                                      <rect
                                        key={`${d}-${h}`}
                                        x={leftPad + h * (cellSize + gap)}
                                        y={topPad + d * (cellSize + gap)}
                                        width={cellSize}
                                        height={cellSize}
                                        rx={2}
                                        fill={colors[level]}
          ...
          (() => {
            ...
            const ranked = rankData
              ? [...rankData]
                  ...
                  .map((r) => {
                    const agentInfo = agents.find((a) => a.agentId === r.agentId);
                    const configAgent = configAgentsRef.current?.get(r.agentId);
                    return {
                      ...r,
                      emoji: agentInfo?.emoji || configAgent?.emoji || '🤖',
                      name: agentInfo?.name || configAgent?.name || r.agentId,
                    };
                  })
              : null;
            return (
                          ...
                          <div key={a.agentId}>
                            ...
                            <div className="w-full h-2 rounded-full bg-[var(--bg)] overflow-hidden">
                              <div
                                ...
                                style={{ width: `${a.idlePercent}%`, backgroundColor: barColor }}
                              />
                            </div>
                            <div className="flex gap-3 text-[10px] text-[var(--text-muted)] mt-0.5">
                              <span>
                                {t('idleRank.online')} {a.onlineMinutes}m
                              </span>
                              <span>
                                {t('idleRank.active')} {a.activeMinutes}m
                              </span>
                              <span>
                                {t('idleRank.idle')} {a.idleMinutes}m
                              </span>
                            </div>
                          </div>
                        );
        ...
        {fullscreenPhoto && photographRef.current && (
          <div
            ...
            onClick={() => {
              if (isMobileViewport && performance.now() - fullscreenPhotoOpenedAtRef.current < 280)
                return;
              setFullscreenPhoto(false);
            }}
          >
            <img
              src={photographRef.current.src}
              alt="photograph"
